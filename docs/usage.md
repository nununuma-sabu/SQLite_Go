...
```text
db > select p.username as name, p.id + 10 display_id from users p;
+------+------------+
| name | display_id |
+------+------------+
| alice | 11         |
...
Oracleの `DUAL` 表に近い仮想テーブルとして `dual` を使えます。
1行だけを持つため、簡単な計算や集約関数の確認に使えます。
`select * from dual;` は `dummy` カラムに `X` を返します。

```text
db > select 1 + 2, (10 - 4) / 3 from dual;
+-------+--------------+
| 1 + 2 | (10 - 4) / 3 |
+-------+--------------+
| 3     | 2            |
